...
uasort($allNotes , "sortNotes");
foreach ($allNotes as $noteID => $noteInfo)
{
	// Get note type
	$noteType = (isset($publicNotes[$noteID]) ? noteManager::PUBLIC_NOTE : noteManager::PRIVATE_NOTE);
	...
	// Delete note form
	$form = new simpleForm();
	$removeLogForm = $form->build("", FALSE)->engageApp("deleteNote")->get();
	DOM::append($ntrow, $removeLogForm);
	HTML::addClass($removeLogForm, "removeNoteForm");
	...
	// Add the remove button only for owner notes
	if ($authorID == account::getAccountID())
	{
		// Submit button
	...
	{
		$authorInfo = account::info($authorID);
		$attr = array();
...
{
	if ($noteA['time_updated'] == $noteB['time_updated'])
		return 0;
